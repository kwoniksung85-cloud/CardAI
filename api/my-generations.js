...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const genRes = await fetch(
      `${SUPABASE_URL}/rest/v1/generations?user_id=eq.${user.id}&order=created_at.desc&limit=${limit}&select=id,title,thumbnail_url,card_count,created_at,cards_json,image_urls_json,threads_text,instagram_caption,theme,topic`,
      { headers: headers() }
    );
